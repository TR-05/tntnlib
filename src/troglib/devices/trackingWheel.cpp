/**
 * @file src/troglib/devices/trackingWheel.cpp
...
#include "troglib/devices/trackingWheel.h"
 ...
 */
troglib::TrackingWheel::TrackingWheel(std::shared_ptr<Encoder> encoder, float diameter, float offset)
    : encoder(encoder),
      diameter(diameter),
      offset(offset) {}
 ...
 */
troglib::TrackingWheel::TrackingWheel(vex::motor_group* motors, float diameter, float offset,  float cartridgeRpm, float rpm)
...
troglib::TrackingWheel::TrackingWheel(vex::triport::port &port, float diameter, float offset, float ratio)
 ...
 */
troglib::TrackingWheel::TrackingWheel(int port, float diameter, float offset, float ratio)
 ...
 */
bool troglib::TrackingWheel::reset() { encoder->reset(); 
 ...
 * (angle / 2) * diameter
 */
float troglib::TrackingWheel::getDistance() { 
  return encoder->getAngle() / 2 * diameter; }

/**
 * Get the offset from the tracking center, in inches
 */
float troglib::TrackingWheel::getOffset() const { return this->offset; }

/**
 * Get the diameter of the wheel, in inches
 */
float troglib::TrackingWheel::getDiameter() const { return this->diameter; }
