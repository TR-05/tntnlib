 ...
 */
troglib::FAPID::FAPID(float kF, float kA, float kP, float kI, float kD) {
    this->kF = kF;
    this->kA = kA;
    this->kP = kP;
    this->kI = kI;
 ...
 * @param kD derivative gain, multiplied by change in error and added to output
 */
void troglib::FAPID::setGains(float kF, float kA, float kP, float kI, float kD) {
    this->kF = kF;
    this->kA = kA;
    this->kP = kP;
    this->kI = kI;
 ...
 * @param maxTime
 */
void troglib::FAPID::setExit(float largeError, float smallError, int largeTime, int smallTime, int maxTime) {
 ...
 * @return float - output
 */
float troglib::FAPID::update(float target, float position, bool log) {
    ...
    if (kA != 0) output = troglib::slew(output, prevOutput, kA);
    prevOutput = output;
    prevError = error;
    totalError += error;
