...
#include "troglib/movements/turn.h"
#include "vex.h"
/**
 * Turn constructor
 *
...
troglib::Turn::Turn(FAPID angularPID, float target, int maxSpeed)
 ...
 * initial competition state.
 */
troglib::Turn::Turn(FAPID angularPID, Pose target, bool reversed, int maxSpeed)
    : angularPID(angularPID),
      targetPose(target),
      reversed(reversed),
      maxSpeed(maxSpeed) {
 ...
 */
float troglib::Turn::getDist() { return dist; }
 ...
 */
std::pair<int, int> troglib::Turn::update(Pose pose) {
