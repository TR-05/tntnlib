 ...
 */
float troglib::slew(float target, float current, float maxChange) {
 ...
 */
float troglib::radToDeg(float rad) { return rad * 180 / M_PI; }
 ...
 */
float troglib::degToRad(float deg) { return deg * M_PI / 180; }
 ...
 */
float troglib::angleError(float angle1, float angle2, bool radians) {
 ...
 * @return float
 */
float troglib::avg(std::vector<float> values) {
    float sum = 0;
 ...
 */
float troglib::ema(float current, float previous, float smooth) {
    return (current * smooth) + (previous * (1 - smooth));
}

float troglib::time(bool millis) {
 ...
 */
float troglib::getCurvature(Pose p1, Pose p2) {
 ...
 */
troglib::Waypoint troglib::closestWaypoint(const std::vector<Waypoint>& waypoints, const Pose& target) {
 ...
 */
troglib::Pose troglib::circleLineIntersect(Pose p1, Pose p2, Pose center, float radius) {
    // uses the quadratic formula to calculate intersection points
    troglib::Pose d = p2 - p1;
    troglib::Pose f = p1 - center;
    float a = d * d;
    float b = 2 * (f * d);
